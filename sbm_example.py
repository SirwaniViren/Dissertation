...
data_input = admissions[['hadm_id', 'congestive_heart_failure', 'cardiac_arrhythmias' , 
...
model = non_hsbm()                                          ## we create an instance of the sbmtm-class
...
entropy=[model.state.multiflip_mcmc_sweep(beta=np.inf, niter=10) for i in range(100)] # In this case we are runing the sweep 10 time with 1000 batches each
model.save_model('SBM_no_deg_core_full_simple_model')
file = open('SBM_no_deg_core_full_simple_model.pickle', 'rb')
...
#output="SBM_full_simple_bipartite_network.svg")
